...
| First request | `docs/getting-started/first-request` | `docs/reference/keyboard-shortcuts`, `bulkEdit.ts` | Good | Filled request editor, headers tab, and send/response flow are captured. Bulk edit is documented; no screenshot yet. |
| Workspaces, collections, folders | `docs/guides/workspaces`, `docs/guides/collection-defaults` | `docs/reference/relay-yaml-format`, `collectionDefaults.ts`, `constants.ts` | Good | Defaults are collection-level only — folders carry no defaults of their own. The inherited settings list is checked against the settings model by a test, so a new setting cannot silently stop inheriting. Keep folder depth and request caps in sync with code. |
