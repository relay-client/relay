...
### Changed
- **The Homebrew cask is generated by a tested script rather than inline in the release.** It gained `auto_updates true` — without it `brew upgrade` would fight Relay's own updater and reinstall whatever version the tap last recorded — plus a `zap` list so an uninstall clears the profile, and a `livecheck` block. CI generates a cask from a stand-in DMG and checks it parses; a cask is otherwise only exercised at install time, on someone else's machine. The channel stays off: the macOS build is not notarized yet, and Homebrew installs with quarantine, so the first launch would fail with "Relay is damaged" rather than an honest unsigned-app prompt.
