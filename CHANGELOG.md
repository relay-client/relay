...
### Fixed
- **The cookie panel's footer sat directly on the list.** The scrolling list had no boundary at either end: the buttons row carried no top padding, so **Clear all cookies** rested on whatever chip happened to be at the bottom, and the first domain card was sliced mid-row by the domain input above it. Both ends of the scroll area are now bounded, so content that continues past the fold reads as scrolled rather than broken.
